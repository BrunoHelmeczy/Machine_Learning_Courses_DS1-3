...
The exercise indicated 70% test to be used. I use createDataPartition() from caret to get row indices, & use them to split the dataset to train- & holdout samples.  


#### 1 c) Lin. Regr. 2 predict logTotalValue -> 10-fold CV to assess predictive power

Below I test an OLS linear regression model, to be used as baseline vs following models below. To fully-define my feature space, I wrote a double-loop to interact all area-related variables, yielding (10 * 9)/2 = 45 interactions in total. I paste these together  
